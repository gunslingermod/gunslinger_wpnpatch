...
asm
  ...
  @nochange:
      ...
      pop dword ptr [esi+$7ac]      //восстанавливаем старый индекс
...
begin
  ...
  // Запрет клина в первом выстреле после перезарядке - надо для протекты
  buf:=GetBuffer(wpn);
  ...
  // на бараьанных дробовиках при "шахматной" зарядке при заклине будет извлекаться гильза не того цвета - если предыдущий выстрел сделан патроном не того типа, клинить нельзя
...
begin
  ...
  if flags=kActPress then begin
    if aim_now then begin
      // Мы сейчас в режиме прицеливания - надо сменить режим
      ...
      // Никакое прицеливание не активно, по возможности используем альтернативное
...
begin
  ...
  if wpn<>nil then begin
    ...
    if (act<>nil) and (GetOwner(wpn)=act) then begin
      // В прицеливании стреляем из центра камеры, иначе - нет
...
////////////////////////////////////////////////////////////////////////////////////
  ...
  // В CWeapon::FireTrace обрабатываем событие уменьшения счетчика патронов
