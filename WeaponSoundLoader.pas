...
type HUD_SOUND_ITEM_SSnd = packed record
  snd:ref_sound;
  delay:single;
  volume:single; // из-за бага в HUD_SOUND_ITEM::PlaySound толком не работает; вырезаем использование там и используем в своих цел€х
                 // отрицательное число значит, что звук "разблокирован". ћодуль определ€ет вариацию частоты
end;
...
//также тут выполн€ем коррекцию частоты дл€ стрельбы
