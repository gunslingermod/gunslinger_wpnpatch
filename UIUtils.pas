...
upgrade_icon_data = packed record
  ...
  enabled:boolean; // true означает, что отображение иконки может быть активно, но при этом icon не всегда ненулевой
                   // (он обнуляется при установке несовместимого аддона и пересоздается после его снятия)
                   // false означает, что соответствующая данному апгрейду иконка (если прописана) не будет показываться
end;
...
function CellItemBuffer._UpdateItemIconStatus(icon:pupgrade_icon_data; section:string; status:boolean):boolean;
begin
  result:=false;
...
begin
  ...
  _elements[i].icon_section:=section;
  _elements[i].offset.x:=offset.x;
...
var
  section, icon_section:PAnsiChar;
  offset:FVector2;
  i:integer;
  element:pupgrade_icon_data;
const
...
//  Log('Update for wpn '+ inttohex(cardinal(_my_item), 8));
  ...
  if (_addons_flags<>new_flags) or (new_scope<>_scope) then begin
    if (new_scope<>nil) and (length(new_scope)=0) then begin
      new_scope:=nil;
    end;
...
begin
  ...
  //В CUIWeaponCellItem::RefreshOffset добавляем оффсеты при изменении размеров и координат иконок
  ...
  // В CUIInventoryCellItem::CUIInventoryCellItem реализуем механизм подмены текстуры с иконками
  ...
  // в InventoryUtilities::DestroyShaders удаляем все созданные нами шейдеры новых текстур иконок
  jmp_addr:=xrGame_addr+$465ab0;
  if not WriteJump(jmp_addr, cardinal(@InventoryUtilities__DestroyShaders_Patch), 5, false) then exit;

  //В CUIWeaponCellItem::CreateDragItem заменяем вызовы InventoryUtilities::GetEquipmentIconsShader() на наши, учитывающие новые текстуры
  ...
  // В CUIWeaponCellItem::CreateIcon заменяем вызовы InventoryUtilities::GetEquipmentIconsShader() на наши, учитывающие новые текстуры
  ...
  //В CGameTaskManager::UpdateTasks исправляем баг - даже когда проводника в Припять еще нет, стрелка на миникарте все равно указывает ня Янов
