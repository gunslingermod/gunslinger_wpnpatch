...
type conditional_breaking_params = packed record
  start_condition:single; //при каком состо€нии начнутс€ проблемы
  end_condition:single;   //при каком состо€нии отрубитс€ вообще
  start_probability:single; //веро€тность проблем в стартовом состо€нии
end;
...
type
  ...
  WpnBuf = class
    ...
    last_frame_rocket_loaded:cardinal; //дл€ –ѕ√
...
begin
  ...
  ammo_cnt_to_reload:=-1;     //только 2стволы, максимальное число зар€жаемых патронов
...
begin
  ...
  if IsActorSuicideNow() or IsActorPlanningSuicide() or IsControllerPreparing() then begin
    ...
    //на случай, если мы недовышли из прицеливани€
    result:=true
...
begin
  //¬ыходить из зума запрещено при действи€х с оружием из-за опасности некорректной работы анимации выхода из прицеливани€
  //Ќо если нам нужен эффект "выхода наполовину" - то разрешать его надо
  //—оответственно, разрешение на "полуприцеливание" надо прописывать в конфиге самого оружи€

  //≈сли на оружии стоит прицел - то мы сначала смотрим, разрешает ли оружие состо€ние "полуприцела", а затем - разрешает ли установленный прицел.
  result:=true;
...
//      result:=true;
    ...
    result:=true;
    //≈сли мы в спринте сейчас - то предварительно надо проиграть аниму выхода из него
...
begin
  ...
  //„итаем все варианты партиклов точки
